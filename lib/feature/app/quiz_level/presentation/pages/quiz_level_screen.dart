...
import '../../../resources/data/model/results_request_model.dart';
...
class QuizLevelScreen extends StatelessWidget {
  ...
  const QuizLevelScreen({super.key, required this.subCategoryId});


  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<LevelQuestionCubit>()..fetchLevelQuestion(subCategoryId),
      child: Builder(
        builder: (context) {
              ...
              child: BlocConsumer<LevelQuestionCubit, LevelQuestionState>(
                listener: (context, state) {
                  if (state.levelQuestionState == Status.completed) {
                    ...
                    final levelText = _getLocalizedLevel(
                      userLevel.toLowerCase(),
                    );
                    DialogUtils.showMessage(
                      ...
                      isError: score <= 33.33,
                      posAction: () {
                        context.read<UpdateUserLevelCubit>().updateUserLevel(
                          levelText,
                        );
